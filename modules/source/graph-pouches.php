    <div class="card">
        ...
        <div class="card-body">
            <form method="GET" class="mb-2" action="/">
                <?php
                // Handle Year Logic
                if (!isset($_GET["pyear"]) || !is_numeric($_GET["pyear"])) {
                    $pouchesyear = date("Y");
                } else {
                    $pouchesyear = $_GET["pyear"];
                }
                ...
                <div class="input-group mb-2">
                    ...
                    <input type="number" name="pyear" class="form-control"
                        value="<?php echo $pouchesyear; ?>"
                ...
                <div class="input-group">
                    ...
                    if ($pouchesmonth == "01") {
                        ...
                        // Added pyear to the link to persist the year selection
                        echo '<a href="/?pyear=' . $pouchesyear . '&pmonth=' . ($pouchesmonth - 1) . '" class="btn btn-outline-light"><i class="bi bi-rewind-fill"></i></a>';
                    }
                    ?>

                    <select onchange="this.form.submit()" class="form-select" name="pmonth" aria-label="Month select">
                        <?php
                        $monthslist = monthsGet();
                        foreach ($monthslist as $monthoption) {
                            $selected = ($monthoption[0] == $pouchesmonth) ? 'selected' : '';
                    ...
                    if ($pouchesmonth == "12") {
                        ...
                        // Added pyear to the link to persist the year selection
                        echo '<a href="/?pyear=' . $pouchesyear . '&pmonth=' . ($pouchesmonth + 1) . '" class="btn btn-outline-light"><i class="bi bi-fast-forward-fill"></i></a>';
                    }
                    ?>
                </div>
            </form>
            ...
            $history = pouchGetHistoryArrayMonthYear($username, $pouchesmonth, $pouchesyear);
            if (sizeof($history) == 0) {
